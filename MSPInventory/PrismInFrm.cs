...
{
    public partial class PrismInFrm : Form
    {
        MainFrm m_frmMain = null;
        DatabaseMgr m_dbMgr = null;

        public PrismInFrm(MainFrm frmMain, DatabaseMgr dbMgr, string strPrismType, string strQuantity, string strLocation)
        {
            InitializeComponent();
            m_frmMain = frmMain;
            m_dbMgr = dbMgr;
            ...
            if (strPrismType != "")
            {
                ...
                cboFrom.SelectedIndex = cboFrom.FindString(strLocation);
            }
        }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    // check if office have some quantity of the prism type; if have, add this to the quantity
                    m_frmMain.AddPrismQuantity(iPrismTypeID, Convert.ToInt32(numQuantity.Value), cboFrom.Text.Trim(),
                        Program.m_strMSPOffice, cboBy.Text.Trim(), dtDateIn.Value);

                    // check if from-location have some quantity of the prism type; if have, deduct this from the quantity;
                    m_frmMain.DeductPrismQuantity(iPrismTypeID, Convert.ToInt32(numQuantity.Value), cboFrom.Text.Trim());
                }
                else
                {
                ...
                //m_frmMain.AddActivity(-1, iPrismTypeID, Convert.ToInt32(numQuantity.Value), cboFrom.Text.Trim(),
                //    Program.m_strMSPOffice, cboBy.Text.Trim(), dtDateIn.Value, txtRemarks.Text);
                m_frmMain.AddActivity(-1,-1, iPrismTypeID, Convert.ToInt32(numQuantity.Value), cboFrom.Text.Trim(),
                Program.m_strMSPOffice, cboBy.Text.Trim(), dtDateIn.Value, txtRemarks.Text);
            }
            catch (Exception ex)
            {
